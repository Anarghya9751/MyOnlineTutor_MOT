...
const fetchData = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/api/discussion/save', discussionFormData);
  ...
  const fetchDiscussions = async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/discussion/getAll');
      if (response.status === 200) {
